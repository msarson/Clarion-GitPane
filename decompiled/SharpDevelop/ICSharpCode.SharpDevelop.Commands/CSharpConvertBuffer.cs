...
public class CSharpConvertBuffer : AbstractMenuCommand
{
	public override void Run()
	{
		...
		IParser parser = ParserFactory.CreateParser(SupportedLanguage.VBNet, new StringReader(((IEditable)activeWorkbenchWindow.ViewContent).Text));
		parser.Parse();
		...
		new VBNetToCSharpConvertVisitor().VisitCompilationUnit(parser.CompilationUnit, null);
		using (SpecialNodesInserter.Install(currentSpecials, cSharpOutputVisitor))
		{
			cSharpOutputVisitor.VisitCompilationUnit(parser.CompilationUnit, null);
		}
		FileService.NewFile("Generated.CS", "C#", cSharpOutputVisitor.Text);
	}
}
